...
abstract class BaseTransactionManager {
  ...
  Future<CheckoutResponse> getCardInfoFrmUI(PaymentCard? currentCard) async {
    PaymentCard? newCard = await showDialog<PaymentCard>(
        ...
        context: context,
        builder: (BuildContext context) => CardInputWidget(card: currentCard));
  ...
  Future<CheckoutResponse> getOtpFrmUI({
    ...
    final resolvedMessage =
        message ?? (response?.displayText?.isNotEmpty == true
    ...
    final String? otp = await showDialog<String>(
      context: context,
      barrierDismissible: false,
      builder: (BuildContext context) => OtpWidget(message: resolvedMessage),
    );
  ...
  Future<CheckoutResponse> getAuthFrmUI(String? url) async {
    ...
    if (result != null) {
      try {
        ...
        debugPrint('Error parsing authorization response: $e');
      }
    }
    return _initApiResponse(apiResponse);
  }
